...
3. Next, we group reaction nodes into reaction groups. Two nodes are in the same group if one of the following is true:
...
4. Optimization pass: We perform action forwarding, as follows: For each node whose expression is dependent on a
   ...
   node.
5. Optimization pass: We perform another round of expression simplification.
6. We convert all ObjectActiveState expressions into InternalParameterConditions, and all ObjectActiveDriver nodes into
   InternalParameterDrivers.
7. We construct PriorityGroups for each target, then group together targets with the same PriorityGroups. However,
...
9. We now assign a depth to each PriorityGroup, with all PriorityGroups that contain non-InternalParameterDrivers at
...
10. We add DelayNodes to the graph to align all nodes at the same point. This is done as follows:
...
11. Finally, we convert the priority nodes to a motion node PriorityNode or BranchNode.