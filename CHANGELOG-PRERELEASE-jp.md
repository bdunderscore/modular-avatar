...
### Added
- [#2084] `位置を元アバターに合わせてリセット`による位置変更をオプションで無効化出来るように

### Fixed
- [#2084] `位置を元アバターに合わせてリセット`において、`衣装の全体的なスケールをアバターに合わせる`オプションが有効な場合に、スケール変更をUndo出来ない問題を修正

### Changed
- [#2084] `位置を元アバターに合わせてリセット`のオプションの項目名、ツールチップを更新
...
### Fixed
- [#1818] VRCSDK がプロジェクトにないときに発生するコンパイルエラーを修正
...
### Removed
- [#1774] `Tools -> Activate PBs` を削除。この項目は初期のテストスクリプトで、含む予定のない項目でした。
