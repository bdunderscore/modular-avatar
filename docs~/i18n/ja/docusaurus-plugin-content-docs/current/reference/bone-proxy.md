...
Bone Proxyは同時に元の位置を指定するすべてのアニメーターを調整し、移動先のパスを指定するように変更します。
...
「親のスケールに合わせる」オプションを使うことで、ターゲットボーンの親スケールに対する
Bone Proxyのローカルスケールを自動的に調整できます。

無効にした場合、Bone Proxyは従来の動作を保持します。つまり、Bone Proxyは
新しいターゲットボーンに移動した後も、元のワールドスケールを保持します。

有効にした場合、ボーンをターゲットボーンに移動する際に、ローカルスケールが
(1, 1, 1)に調整されます。Bone Proxyを使ってアバターのボーンにアクセサリーをつけるなど、ターゲットボーンが
スケールされている可能性がある場合でも同じ相対的なサイズを保つようにしたい時に便利です。