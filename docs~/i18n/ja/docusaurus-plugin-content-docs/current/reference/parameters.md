...
MA Parametersリストの各エントリーは、一つのパラメーター、またはVRChatのPhysBoneの接頭辞を指定します。上段に名前（または接頭辞）を、
その横にパラメーターの型を設定します。
...
「接頭辞」設定は、PhysBonesやRaycastコンポーネントにParameterとして設定されている場合に使います。
PhysBonesやRaycastコンポーネントでは、接頭辞を頭に複数のパラメーターを生成するので、これで一括に設定することができます。
Animatorのみと同様、Expressions Parametersリストには追加されません。
