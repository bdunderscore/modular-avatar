...
「マッピング」の曲線を調整することで、同期先のブレンドシェープの反応を調整できます。たとえば、元のブレンドシェープと
逆の動きをさせることもできます。アニメーションカーブと同様に、適切なタンジェントを設定する必要があることにご注意ください。

複数のオブジェクトを選択して、ひとくくりでの編集も対応しています。

Blendshape Syncは複数レベルのオブジェクトを経由しての同期には対応していないことに注意してください（オブジェクトA→オブジェクトB→オブジェクトCなど）。
### 細かい仕様
