...
「子オブジェクトの位置を調整」のチェックボックスは、親のスケールが変わるときに子ボーンの相対位置を調整するかどうかを制御します。この設定はエディター全体に適用され、個々のコンポーネントごとではなく、すべてのScale Adjusterコンポーネントに影響します。また、Unityを再起動するとリセットされます。子ボーンを移動させずに親のスケールを調整したい場合に便利です。なお、子ボーンの位置のみが調整されます。子ボーンのスケールは変更されません。
