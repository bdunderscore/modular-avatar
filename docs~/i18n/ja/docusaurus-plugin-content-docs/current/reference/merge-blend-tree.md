...
モーションを無効にしたり、時間に応じて変化するアニメーションの場合はMerge Motionは使用しないでください。

:::info

Merge Motionは、すべてのモーションを一つの Direct Blend Tree に統合します。Unityでは、そのようなモーションはすべて同じ
再生時間になるように再生速度が調整され、最終的な再生速度が不定になります。そのため、時間で変化するアニメーションでは
Merge Motionではなく、[Merge Animator](merge-animator.md)を使用してください。
