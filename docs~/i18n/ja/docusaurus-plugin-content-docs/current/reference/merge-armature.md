...
Setup Outfitで設定された衣装では、衣装の[Outfit Root](outfit-root.md)コンポーネントからも同じ調整ツールを使用できます。

## Scale Adjusterを元アバターに合わせる

「Scale Adjusterを元アバターに合わせる」ボタンは、対応する各ボーンについて、素体の[Scale Adjuster](scale-adjuster.md)設定を衣装へコピーします。
素体にScale Adjusterがある場合は衣装側で追加または更新し、素体にない場合は衣装側から削除します。
...
Merge Armatureがボーンを名前で照合するので、つけるだけでは非対応衣装がうまく動かない場合があります。
