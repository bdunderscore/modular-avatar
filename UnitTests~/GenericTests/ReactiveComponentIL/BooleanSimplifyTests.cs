...
namespace UnitTestsReactiveComponentIL
{
    public class BooleanSimplifyTests : TestBase
    {
        ...
        [Test]
        public void AndNode_WithSameNameButDistinctParameterConditions_RemainsDistinct()
        {
            var greaterThanQuarter = new ParameterExpression("test", 0.25f, ParameterExpression.ConditionMode.GreaterThan);
            var greaterThanThreeQuarters = new ParameterExpression("test", 0.75f, ParameterExpression.ConditionMode.GreaterThan);
            var lessThanQuarter = new ParameterExpression("test", 0.25f, ParameterExpression.ConditionMode.LessThan);
            var expr = new AndNode(greaterThanQuarter, greaterThanThreeQuarters, lessThanQuarter);

            var result = SimplifyExpression(expr);

            Assert.AreEqual(
                new AndNode(greaterThanQuarter, greaterThanThreeQuarters, lessThanQuarter),
                result
            );
        }
