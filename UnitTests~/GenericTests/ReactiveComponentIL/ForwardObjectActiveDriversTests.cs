...
namespace UnitTestsReactiveComponentIL
{
    public class ForwardObjectActiveDriversTests : TestBase
    {
        ...
        [Test]
        public void SharedDriverExpression_DoesNotForwardIntoItself()
        {
            var obj = CreateRoot("obj");
            var sharedExpression = new AndNode(
                new ObjectActiveState(obj, ObjectActiveState.State.Active),
                new ParameterExpression("p1")
            );
            var consumer = new ReactionNode(sharedExpression, new NullAction(new object()));
            var driver = new ReactionNode(sharedExpression, new DriveActiveState(obj, true));
            var graph = new ReactionGraph();
            ...
            Assert.AreSame(sharedExpression, consumer.Expression);
            Assert.AreSame(sharedExpression, driver.Expression);
        }

        [Test]
        public void RepeatedObjectActiveStates_ReceiveDistinctDeepClones()
        {
            var obj = CreateRoot("obj");
            var driverExpression = new AndNode(
                new ParameterExpression("p1"),
                new NotNode(new ParameterExpression("p2"))
            );
            var consumer = new ReactionNode(
                new OrNode(
                    new ObjectActiveState(obj, ObjectActiveState.State.Active),
                    new ObjectActiveState(obj, ObjectActiveState.State.Active)
                ),
            ...
            var graph = new ReactionGraph();
            graph.AddNode(consumer);
            graph.AddNode(new ReactionNode(driverExpression, new DriveActiveState(obj, true)));

            ForwardObjectActiveDriversTransform.Apply(graph);

            var replacements = (OrNode)consumer.Expression;
            var first = (AndNode)replacements.Children[0];
            var second = (AndNode)replacements.Children[1];
            Assert.AreEqual(driverExpression, first);
            Assert.AreEqual(driverExpression, second);
            Assert.AreNotSame(driverExpression, first);
            Assert.AreNotSame(driverExpression, second);
            Assert.AreNotSame(first, second);
            Assert.AreNotSame(first.Children[0], second.Children[0]);
        }
    }
}
