...
public class VertexFilterByBoneTests : TestBase
{
    private SkinnedMeshRenderer CreateTestRenderer(BoneWeight1[] boneWeights, byte[] bonesPerVertex, Transform[] bones)
    {
        ...
        // Degenerate triangles (one per vertex) make the primitive mask directly reflect the
        // vertex mask produced by the bone filter.
        var tris = new int[vertexCount * 3];
        for (int i = 0; i < vertexCount; i++)
            tris[i * 3] = tris[i * 3 + 1] = tris[i * 3 + 2] = i;
        mesh.triangles = tris;
    ...
    public void VertexFilterByBoneBasicTest()
    {
        ...
        var filtered1 = RunFilterPrimitives(filter1, renderer, mesh);
    ...
    public void VertexFilterByBoneAllVerticesModeTest()
    {
        ...
        var bonesPerVertex = new byte[] { 1, 1, 1 };

        var renderer = CreateTestRenderer(boneWeights, bonesPerVertex, bones);
        var mesh = renderer.sharedMesh;
        mesh.triangles = new[] { 0, 1, 2 };

        var anyVertexFilter = new VertexFilterByBone(bone0, 0.5f, VertexSelectionMode.AnyVertex);
        var allVerticesFilter = new VertexFilterByBone(bone0, 0.5f, VertexSelectionMode.AllVertices);

        CollectionAssert.AreEqual(new[] { true }, RunFilterPrimitives(anyVertexFilter, renderer, mesh));
        CollectionAssert.AreEqual(new[] { false }, RunFilterPrimitives(allVerticesFilter, renderer, mesh));
    }
    ...
    public void VertexFilterByBoneWithNullBone()
    {
        ...
        Assert.AreEqual(0, filtered.Count(x => x), "Null bone should not filter any vertices");
    }
    ...
    public void VertexFilterByBoneObjectRegistryReplacementTest()
    {
        ...
        var bonesPerVertex = new byte[] { 1, 2, 1 }; // 3 vertices
